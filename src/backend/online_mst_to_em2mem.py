...
from online_short_term.mst_to_episodic import build_episodic_from_mst, update_mst_em2mem_metadata
from online_short_term.schemas import DEFAULT_SESSIONS_ROOT


def consolidate_short_term_to_em2mem(
    *,
    session_id: str,
    ...
    if dry_run or not update_em2mem:
        result["updated_em2mem"] = False
        result["em2mem_update_mode"] = None
        return result
    if generation_backend:
        import os
        os.environ["EM2MEM_MEMORY_GENERATION_BACKEND"] = generation_backend
    ...
    if os.getenv("EM2MEM_ALLOW_FULL_REBUILD_FALLBACK", "0").strip().lower() in {"1", "true", "yes", "on"} and os.getenv("EM2MEM_MEMORY_UPDATE_MODE", "").strip().lower() in {"full_rebuild", "full_rebuild_fallback"}:
        config_path = build_online_em2mem_memory(
            session_id=session_id,
            sessions_root=Path(sessions_root),
            force=True,
        ...
        update_mst_em2mem_metadata(session_dir, update_mode="full_rebuild_fallback")
        result["em2mem_update_mode"] = "full_rebuild_fallback"
        ...
        result["em2mem_update_mode"] = append_result.em2mem_update_mode
        result["memory_config_path"] = "em2mem/memory_config.json"
        result["incremental_append"] = append_result.to_dict()
    result["updated_em2mem"] = True
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="Build MST 30s episodic memory and optionally feed it into the Em2Mem adapter.")
    ...
    parser.add_argument("--update-em2mem", action="store_true")
    ...
    parser.add_argument("--generation-backend", default=None, choices=["llm", "rule"], help="Em2Mem 30s+ memory generation backend. Defaults to EM2MEM_MEMORY_GENERATION_BACKEND or llm.")
    ...
    result = consolidate_short_term_to_em2mem(
        ...
        update_em2mem=args.update_em2mem,
        force=args.force,
