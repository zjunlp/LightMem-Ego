...
echo "[start_online_visual_worker] EM2MEM_VISUAL_BACKEND=${EM2MEM_VISUAL_BACKEND:-vlm2vec}"
echo "[start_online_visual_worker] EM2MEM_CLEAN_CUDA_ENV=${EM2MEM_CLEAN_CUDA_ENV}"
...
echo "[start_online_visual_worker] EM2MEM_VLM2VEC_MODEL_PATH=${EM2MEM_VLM2VEC_MODEL_PATH}"
echo "[start_online_visual_worker] EM2MEM_ALLOW_HF_DOWNLOAD=${EM2MEM_ALLOW_HF_DOWNLOAD}"
if [[ "${EM2MEM_VISUAL_BACKEND:-vlm2vec}" == "remote" ]]; then
  python - <<'PY'
import sys
