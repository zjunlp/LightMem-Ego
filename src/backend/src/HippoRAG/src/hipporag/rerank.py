...
class DSPyFilter:
    ...
    def rerank(self,
        ...
        try:
            ...
            print(f"[DSPyFilter] candidate_items={len(candidate_items)}")
            response = self.llm_call(query, json.dumps(fact_before_filter))
            print(f"[DSPyFilter] raw response:\n{response[:1000] if isinstance(response, str) else response}")
            generated_facts = self.parse_filter(response)
            print(f"[DSPyFilter] generated_facts={len(generated_facts)} -> {generated_facts[:5]}")
        except Exception as e:
            print('exception', e)
            generated_facts = []
        result_indices = []
