...
class OpenAIModel:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if timeout is None:
            timeout = _env_float("WORLDMM_OPENAI_TIMEOUT_SECONDS", 120.0)
    ...
    def generate(self, prompt: Union[str, List[Dict[str, Any]]], text_format: Optional[type] = None, **kwargs) -> Any:
        ...
        if _env_bool("WORLDMM_OPENAI_FORCE_CHAT_COMPLETIONS", True):
            return self._generate_with_chat_fallback(
                processed_prompt,
                text_format=text_format,
    ...
    def _generate_with_chat_fallback(
        ...
        **kwargs: Any,
        ...
        try:
            max_attempts = max(1, int(os.getenv("WORLDMM_CHAT_COMPLETIONS_FALLBACK_ATTEMPTS", "2") or 2))
        except ValueError:
            max_attempts = 2
