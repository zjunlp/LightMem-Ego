...
def _resolve_base_model_name() -> str:
    for env_name in ("WORLDMM_QWEN2VL_BASE_MODEL_PATH", "WORLDMM_VLM2VEC_BASE_MODEL_PATH"):
        value = os.getenv(env_name, "").strip()
        if value:
            return value

    local_path = PROJECT_ROOT / "models" / "Qwen2-VL-2B-Instruct"
...
class VLM2VecV2EmbeddingModel:
    ...
    def get_embedding_dimension(self) -> int:
        ...
        with torch.no_grad():
            test_input = Qwen2_VL_process_fn(
                {"text": ["test"], "images": [None]},
                self.processor,
            )
            for key in ("pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw"):
                test_input[key] = None
            test_input = batch_to_device(test_input, self.device)
