...
def _hf_local_files_only() -> bool:
    return os.getenv("WORLDMM_ALLOW_HF_DOWNLOAD", "0").strip().lower() not in {"1", "true", "yes"}
...
def load_processor(model_args, data_args=None):
    ...
    if model_args.model_backbone in [QWEN2_VL]:
        ...
        image_processor = Qwen2VLImageProcessor.from_pretrained(model_name_or_path, size=size, local_files_only=local_files_only)
        tokenizer = Qwen2Tokenizer.from_pretrained(model_name_or_path, local_files_only=local_files_only)
        processor = Qwen2VLProcessor.from_pretrained(
            model_name_or_path,
            image_processor=image_processor, tokenizer=tokenizer, size=size, local_files_only=local_files_only
        )
        ...
        from transformers import Qwen2Tokenizer
        image_processor = Qwen2VLImageProcessor.from_pretrained(model_name_or_path, local_files_only=local_files_only)
        if data_args is not None:
...
def Qwen2_VL_process_fn(model_inputs: dict, processor: Qwen2VLProcessor, max_length=None):
    ...
    # (2) stack trace from the VLM2Vec pooling path
