...
def _vlm2vec_attn_implementation() -> str:
    return os.getenv("WORLDMM_VLM2VEC_ATTN_IMPLEMENTATION", "eager").strip() or "eager"


def _get_config_attr(config, key: str):
    try:
...
def _load_local_config_dict(model_name_or_path: str) -> dict:
...
def _set_config_attr_if_missing(config, key: str, value):
    if value is not None and _get_config_attr(config, key) is None:
        setattr(config, key, value)


def _prepare_qwen2vl_config(config, model_name_or_path: str | None = None):
    raw_config = _load_local_config_dict(model_name_or_path) if model_name_or_path else {}
    for key, value in raw_config.items():
        if key == "vision_config":
            continue
        _set_config_attr_if_missing(config, key, value)

    raw_vision_config = raw_config.get("vision_config") or {}
    vision_config = _get_config_attr(config, "vision_config")
    if vision_config is not None:
        for key, value in raw_vision_config.items():
            attr_name = "in_channels" if key == "in_chans" else key
            _set_config_attr_if_missing(vision_config, attr_name, value)

    if _get_config_attr(config, "pad_token_id") is None:
        config.pad_token_id = getattr(config, "bos_token_id", None) or getattr(config, "eos_token_id", None)
    rope_scaling = _get_config_attr(config, "rope_scaling")
...
class MMEBModel(nn.Module):
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_backbone in [QWEN2_VL]:
            attn_implementation = _vlm2vec_attn_implementation()
            config = _prepare_qwen2vl_config(config, model_args.model_name)
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        lora_name_or_path = model_args.checkpoint_path if model_args.checkpoint_path else model_args.model_name
        local_files_only = os.getenv("WORLDMM_ALLOW_HF_DOWNLOAD", "0").strip().lower() not in {"1", "true", "yes"}
        config = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=True, local_files_only=local_files_only)
        ...
        if model_args.model_backbone in {QWEN2_VL, QWEN2_VL_TOKENSELECTION}:
            attn_implementation = _vlm2vec_attn_implementation()
            config = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=True, local_files_only=local_files_only)
            config = _prepare_qwen2vl_config(config, model_name_or_path)
            config._attn_implementation = attn_implementation
        ...
        if model_args.lora:
            print_master(f'Loading LoRA from {lora_name_or_path}')
            lora_config = LoraConfig.from_pretrained(lora_name_or_path, local_files_only=local_files_only)
            lora_model = PeftModel.from_pretrained(base_model, lora_name_or_path, config=lora_config, is_trainable=is_trainable, local_files_only=local_files_only)
            if not is_trainable:
