...
class Qwen2VLProcessor(ProcessorMixin):
    ...
    def get_possibly_dynamic_module(module_name):
        # Keep local remote-code classes working with newer Transformers processor
        # validation. We first resolve our vendored classes directly, then fall
        # back to the upstream module registry.
        local_modules = {
        ...
        if hasattr(transformers, module_name):
            return getattr(transformers, module_name)

        try:
            from transformers.processing_utils import ProcessorMixin as _ProcessorMixin

            return _ProcessorMixin.get_possibly_dynamic_module(module_name)
        except Exception as exc:
            raise ValueError(
                f"Could not resolve module {module_name} for Qwen2-VL processor."
            ) from exc
