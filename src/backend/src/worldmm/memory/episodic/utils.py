...
class NerRawOutput(BaseModel):
    ...
    @classmethod
    def accept_bare_entity_list(cls, value: Any) -> Any:
        if isinstance(value, list):
            return {"named_entities": value}
        if isinstance(value, dict) and "named_entities" not in value:
            for key in ("entities", "entity_list"):
                if key in value:
                    return {**value, "named_entities": value[key]}
        return value
...
class TripleRawOutput(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def accept_bare_triple_list(cls, value: Any) -> Any:
        if isinstance(value, list):
            return {"triples": value}
        if isinstance(value, dict) and "triples" not in value and "triplets" in value:
            return {**value, "triples": value["triplets"]}
        return value
