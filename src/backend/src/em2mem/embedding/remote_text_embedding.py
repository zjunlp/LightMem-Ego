...
def _text_embed_url() -> str:
    return (
        os.getenv("EM2MEM_TEXT_EMBED_URL")
        or f"http://{os.getenv('EM2MEM_TEXT_EMBED_HOST', '127.0.0.1')}:{os.getenv('EM2MEM_TEXT_EMBED_PORT', '18096')}"
    ).rstrip("/")
...
class RemoteTextEmbeddingModel:
    ...
    def __init__(
        ...
        del device
        self.model_name = model_name or os.getenv("EM2MEM_TEXT_EMBED_MODEL") or ""
        self.remote_url = (url or _text_embed_url()).rstrip("/")
        self.timeout_seconds = float(
            timeout_seconds
            if timeout_seconds is not None
            else _env_float("EM2MEM_TEXT_EMBED_TIMEOUT_SECONDS", 300.0)
        )
        self.default_batch_size = int(
            default_batch_size
            if default_batch_size is not None
            else _env_int("EM2MEM_TEXT_EMBED_BATCH_SIZE", 256)
        )
        if normalize is None:
            normalize = os.getenv("EM2MEM_TEXT_EMBED_NORMALIZE", "0").strip().lower() in {"1", "true", "yes", "on"}
    ...
    def ping_remote(self) -> dict:
        deadline = time.monotonic() + _env_float("EM2MEM_TEXT_EMBED_READY_TIMEOUT_SECONDS", 900.0)
        poll_seconds = max(0.2, _env_float("EM2MEM_TEXT_EMBED_READY_POLL_SECONDS", 2.0))
