...
from .evidence_to_em2mem import (
    build_caption_items,
...
from .em2mem_layout import Em2MemOnlineLayout, ensure_em2mem_layout
...
def _build_visual_embeddings(
    ...
    try:
        ...
        raise RuntimeError(f"Failed to import Em2Mem EmbeddingModel: {exc}") from exc

    model_name = os.getenv("EM2MEM_VIS_EMBED_MODEL") or os.getenv("EM2MEM_VLM2VEC_MODEL_PATH") or "VLM2Vec/VLM2Vec-V2.0"
...
def build_online_em2mem_memory(
    session_id: str,
    sessions_root: Path,
    force: bool = False,
    ...
    layout = Em2MemOnlineLayout(session_dir=session_dir, session_id=session_id)
    model = model_name or os.getenv("OPENAI_MODEL") or os.getenv("EM2MEM_MEMORY_MODEL") or "gpt-5"
    log_path = layout.logs_root / "em2mem_memory_adapter.log"
    pipeline_mode = os.getenv("EM2MEM_PIPELINE_MODE", "mst").strip().lower()
    ...
    mst_episodes_path = session_dir / "em2mem" / "mst_episodic" / "mst_30sec_episodes.json"
    ...
    legacy_fallback_allowed = os.getenv("EM2MEM_ALLOW_LEGACY_EVIDENCE_FALLBACK", "0").strip().lower() in {"1", "true", "yes", "on"}
    ...
    if source == "mst_episodic":
        ...
        em2mem_update_mode = "full_rebuild_fallback"
        ...
        em2mem_update_mode = "full_rebuild"
    else:
        raise ValueError("source must be auto, online_evidence/legacy_evidence, or mst_episodic")
    memory_generation_backend = _memory_generation_backend(os.getenv("EM2MEM_MEMORY_GENERATION_BACKEND"))
    ...
    if skip_episodic_sidecar:
        _append_log(log_path, "episodic sidecar skipped by EM2MEM_SKIP_EPISODIC_SIDECAR")
    else:
    ...
    config = {
        ...
        "active_30s_source": Path(captioned_30sec_rel).stem,
        "episodic_source": episodic_source,
        "em2mem_30s_input_source": Path(captioned_30sec_rel).stem,
        "em2mem_update_mode": em2mem_update_mode,
        "legacy_evidence_available": legacy_ready,
        ...
        "source": {
            ...
            "adapter": "online_em2mem_memory_adapter",
            "episodic_source": episodic_source,
        },
        "counts": {
        ...
        "query_rag_args": {
            ...
            "respond_model": os.getenv("EM2MEM_RESPOND_MODEL", model),
        ...
        config.update(
            {
                ...
                "last_mst_em2mem_update_at": utc_now_iso(),
            }
        )
    else:
        config.update(
            {
                "mst_episodic_ready": mst_ready,
                "mst_episodic_path": "em2mem/mst_episodic/mst_30sec_episodes.json" if mst_episodes_path.exists() else None,
