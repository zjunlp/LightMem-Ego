...
from .worldmm_layout import WorldMMOnlineLayout, hhmmssff_to_seconds, seconds_to_hhmmssff
...
def _memory_generation_backend(value: str | None = None) -> str:
    backend = (value or os.getenv("WORLDMM_MEMORY_GENERATION_BACKEND") or "llm").strip().lower()
    if backend not in {"llm", "rule"}:
        raise ValueError("WORLDMM_MEMORY_GENERATION_BACKEND must be 'llm' or 'rule'")
    return backend
...
def _build_llm(model_name: str) -> Any:
    ...
    return LLMModel(model_name=model_name, max_retries=_env_int("WORLDMM_MEMORY_LLM_RETRIES", 3))
...
def _openie_triplets_for_items(items: list[dict[str, Any]], model_name: str, output_dir: Path) -> dict[str, list[list[str]]]:
    ...
    _ner_map, triple_map = openie.batch_openie(
        ...
        max_workers=_env_int("WORLDMM_OPENIE_MAX_WORKERS", 4),
    )
...
def _write_llm_multiscale_caption_files(
    ...
    from worldmm.memory.episodic.gen_multiscale import gen_multiscale

    llm = _build_llm(model_name)
    write_json(layout.caption_30sec_path, caption_30s)
    if os.getenv("WORLDMM_MEMORY_FORCE_LLM_REGENERATE", "1").strip().lower() in {"1", "true", "yes", "on"}:
    ...
    gen_multiscale(
        ...
        perspective=os.getenv("WORLDMM_MULTISCALE_PERSPECTIVE", "egocentric"),
        default_date="DAY1",
    )
    return (
...
def _write_semantic_files_with_llm(
    ...
    from worldmm.memory.semantic import SemanticConsolidation, SemanticExtraction
    ...
    extraction = SemanticExtraction(llm_model=_build_llm(model_name), max_retries=_env_int("WORLDMM_SEMANTIC_LLM_RETRIES", 2))
    payload_batch = {}
    ...
    write_json(memory_path, {
        "facts": facts,
        "timeline": timeline,
        "source": "worldmm_llm_semantic_extraction_consolidation",
